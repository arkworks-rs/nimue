...
impl<F: Field, H: DuplexSpongeInterface, R: RngCore + CryptoRng> ProverMessageField<F>
...
impl<C: FpConfig<N>, H: DuplexSpongeInterface<Fp<C, N>>, R: RngCore + CryptoRng, const N: usize>
    ProverMessageField<Fp<C, N>> for ProverPrivateState<H, Fp<C, N>, R>
{
...
impl<G, H, R> ProverMessageGroup<G> for ProverPrivateState<H, u8, R>
where
    G: CurveGroup,
    H: DuplexSpongeInterface,
    R: RngCore + CryptoRng,
    ProverPrivateState<H, u8, R>: CommonProverMessageGroup<G, Repr = Vec<u8>>,
{
    #[inline(always)]
...
where
    ...
    R: RngCore + CryptoRng,
    ProverPrivateState<H, Fp<C, N>, R>: CommonProverMessageGroup<G> + ProverMessageField<G::BaseField>,
{
    #[inline(always)]
    fn add_points(&mut self, input: &[G]) -> ProofResult<()> {
