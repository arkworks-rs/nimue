...
pub trait Absorbs<L: Lane> {
    fn append_element<A: Absorbable<L>>(&mut self, e: &A) -> Result<&mut Self, InvalidTag>;
}
...
where
    ...
    R: RngCore + CryptoRng,
{
    fn append_element<A: Absorbable<S::L>>(&mut self, input: &A) -> Result<&mut Self, InvalidTag> {
